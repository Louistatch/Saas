...
interface CropResult {
  ...
  kpis: {
    total_m3: number
    total_boost_m3: number
    total_optimal_m3: number
    pic_mois: string
    pic_volume_m3: number
    debit_pompe_ls: number
    avg_kc: number
    avg_etp_mmj: number
    max_besoin_net: number
    efficiency_pct: number
    mois_pluie_couvre: string[]
    nb_mois_zero: number
  }
}
interface CalcResult {
  ...
  combined_kpis: {
    ...
    total_survival_m3: number
    total_boost_m3: number
    total_optimal_m3: number
    pic_mois: string
    pic_optimal_m3: number
    debit_pompe_ls: number
  }
}
  ...
  const calculate = async () => {
    ...
    try {
      const payload: Record<string, unknown> = {
        ...
        crops: entries.map((e) => ({
          name: e.crop.name,
          area_m2: Number.parseFloat(e.area_m2) || 1000,
        })),
        soil_type: soil.name,
        system: system.id,
      }
      ...
      const totalSuperficie =
        entries.reduce((sum, e) => sum + (Number.parseFloat(e.area_m2) || 0), 0) / 10000
      ...
      setSavedCalcs((prev) => {
        ...
        try {
          localStorage.setItem('agrismart_history', JSON.stringify(updated))
        } catch {}
        return updated
      })
    } catch (e: unknown) {
    ...
    return (
      <div className="vfp-card rounded-2xl p-6 flex flex-col items-center gap-3 text-center">
        ...
        <button
          ...
          className="text-xs text-[var(--vfp-accent)] underline"
        >
          Réessayer
        </button>
      </div>
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex items-center gap-3">
        ...
        {step > 1 && (
          <button
            type="button"
            onClick={reset}
            className="ml-auto flex items-center gap-1 text-white/30 text-xs active:opacity-60"
          >
      ...
      <div className="flex items-center gap-2">
        ...
        <div>
          ...
          <p className="text-white/35 text-[10px] mt-0.5">
            Bilan hydrique FAO-56 · NASA POWER · ROSETTA v3
          </p>
        </div>
      </div>
      ...
      <div className="space-y-1">
        <div className="flex gap-1">
          {[1, 2, 3, 4, 5].map((n) => (
            <div
              key={n}
              className={`h-1 flex-1 rounded-full transition-all duration-300 ${n <= step ? 'bg-[var(--vfp-accent)]' : 'bg-white/[0.08]'}`}
            />
          ))}
        </div>
        <div className="flex gap-1">
          {STEP_LABELS.map((label, i) => (
            <p
              key={label}
              className={`flex-1 text-center text-[9px] font-semibold transition-colors ${i + 1 === step ? 'text-[var(--vfp-accent)]' : 'text-white/20'}`}
            >
              {label}
            </p>
          ))}
      ...
      {step === 1 && (
        <div className="space-y-3">
              ...
              onClick={async () => {
                try {
                    ...
                    .map((p) => {
                      ...
                      const found = allCrops.find(
                        (c) =>
                          ...
                          cropName.includes(c.name.toLowerCase()),
                      )
                      return found
                        ? {
                            crop: found,
                            area_m2: String(Math.round((p.superficie_ha ?? 0) * 10000)),
                          }
                        : null
                    })
                  ...
                  if (newEntries.length > 0) setEntries(newEntries)
                    ...
                    alert('Aucune culture reconnue dans vos parcelles. Sélectionnez manuellement.')
                } catch {
                  alert('Impossible de charger vos parcelles.')
                }
              }}
          ...
          {entries.length > 0 && (
            <div className="space-y-2">
              {entries.map((entry, idx) => (
                <div
                  key={entry.crop.id}
                  className="vfp-card rounded-2xl p-3 flex items-center gap-3"
                >
                  <div
                ...
                >
                  <div>
                    <p className="text-white text-sm font-medium">
                      {calc.crop} — {calc.region}
                    </p>
                    <p className="text-white/40 text-xs">
                      {calc.superficie} ha ·{' '}
                      {calc.result_daily_mm != null
                        ? `${calc.result_daily_mm.toFixed(1)} mm/j`
                        : '—'}
                    </p>
                  </div>
                  <span className="text-white/25 text-xs">
                    {new Date(calc.id).toLocaleDateString('fr-FR', {
                      day: '2-digit',
                      month: 'short',
                    })}
                  </span>
          ...
          <div ref={cropPickerRef} className="relative">
            ...
            {pickOpen && (
              <div
                className="absolute top-full left-0 right-0 mt-1 z-30 vfp-card rounded-2xl p-2 grid grid-cols-2 gap-1.5 shadow-2xl border border-white/[0.08]"
                style={{ background: 'oklch(0.14 0.04 142)' }}
              >
                {allCrops.map((c) => (
                  <button
                    type="button"
                  ...
                  >
                    ...
                    {alreadySelected(c.id) && (
                      <span className="ml-auto text-[var(--vfp-accent)] text-xs">✓</span>
                    )}
                  </button>
                ))}
              </div>
          ...
          {entries.length === 0 && (
            <p className="text-white/25 text-xs text-center">
              Ajoutez au moins une culture pour continuer
            </p>
          )}
        </div>
      )}
      ...
      {step === 2 && (
        <div className="space-y-2">
            ...
            >
              ...
              <div className="text-right shrink-0 space-y-0.5">
                <p className="text-white font-bold text-sm">
                  {s.RU} <span className="text-xs text-white/30 font-normal">mm/m</span>
                </p>
          ...
          <p className="text-white/20 text-[10px] text-center mt-1">
            ROSETTA v3 · USDA-ARS · van Genuchten
          </p>
        </div>
      )}
      ...
      {step === 3 && (
        <div className="space-y-2">
          <p className="text-white/60 text-sm font-medium mb-3">
            Quel système d'irrigation utilisez-vous ?
          </p>
          {systems.map((s) => (
            <button
              type="button"
            ...
            >
              ...
              <div className="text-right shrink-0">
                <p
                  className={`font-bold text-lg ${s.efficiency >= 0.85 ? 'text-[var(--vfp-accent)]' : 'text-white/70'}`}
                >
        ...
        <div className="space-y-4">
          <div>
            ...
            <p className="text-white/30 text-xs mt-0.5">
              Le GPS permet d'utiliser les normales climatiques précises de votre site (NASA POWER
              30 ans).
            </p>
          </div>
          ...
          >
            {gpsLoading ? (
              ...
              <MapPin
                className={`h-5 w-5 shrink-0 ${gpsCoords ? 'text-[var(--vfp-accent)]' : 'text-white/40'}`}
              />
            )}
            <div className="flex-1 text-left">
              <p className="text-white font-semibold text-sm">
                {gpsCoords
                  ? `GPS · ${gpsCoords.lat.toFixed(3)}°N, ${gpsCoords.lon.toFixed(3)}°E`
            ...
            <div className="grid grid-cols-1 gap-1.5">
              {REGIONS.map((r) => (
                ...
                >
                  ...
                  {region === r.id && (
                    <div className="w-2 h-2 rounded-full bg-[var(--vfp-accent)]" />
                  )}
                </button>
              ))}
            </div>
          ...
          {calcError && (
            <p className="text-red-300/80 text-xs text-center bg-red-500/10 rounded-xl p-3">
              {calcError}
            </p>
          )}
        </div>
      ...
      {step === 5 && result && (
        <div className="space-y-4">
            ...
            <button
              ...
              onClick={() => {
                ...
                result.results.forEach((r) => {
                  const entry = entries.find((e) => e.crop.name === r.crop)
                  lines.push(
                    `${entry?.crop.emoji ?? '🌿'} *${r.crop}* — ${(r.area_m2 / 10000).toFixed(2)} ha`,
                  )
                })
                lines.push(`📍 Région : ${region || 'GPS'}`)
                lines.push(
                  `🌡️ Température moy. : ${result.avg_temp}°C | Pluie : ${result.total_precip} mm/an`,
                )
          ...
          <button
            type="button"
            onClick={reset}
            className="w-full py-3 rounded-xl border border-[var(--vfp-accent)]/20 text-[var(--vfp-accent)] text-sm font-semibold active:opacity-70 flex items-center justify-center gap-2"
          >
...
function KpiCard({
  ...
  return (
    <div
      className={`rounded-xl p-3 border ${accent ? 'bg-[var(--vfp-accent)]/[0.07] border-[var(--vfp-accent)]/20' : 'bg-white/[0.03] border-white/[0.05]'}`}
    >
      <div className="flex items-center gap-1 mb-0.5">
        {icon && (
          <span className={accent ? 'text-[var(--vfp-accent)]' : 'text-white/30'}>{icon}</span>
        )}
        <p className="text-white/35 text-[10px] leading-tight">{label}</p>
      </div>
      <p
        className={`text-lg font-bold leading-tight ${accent ? 'text-[var(--vfp-accent)]' : 'text-white'}`}
      >
