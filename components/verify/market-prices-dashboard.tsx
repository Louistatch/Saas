...
import { TrendingUp, MapPin, PhoneCall } from 'lucide-react'
...
const MARKETS_BY_REGION: Record<string, string[]> = {
  'b0f3fef0-032d-4566-9b97-89d9efcbe23b': ['Lomé (Grand Marché)', 'Tsévié', 'Aného', 'Vogan', 'Tabligbo', 'Kévé'],
  '913db44f-9095-4ee0-8574-8ecbfad47a4a': ['Kpalimé', 'Atakpamé', 'Notsé', 'Badou', 'Amlamé', 'Anié'],
  '1fba6fc3-28e8-48f7-bf3e-774fce7bd9f0': ['Sokodé', 'Sotouboua', 'Blitta', 'Tchamba'],
  'e7becd6d-4f6e-4cb9-9f4d-f70d736800b1': ['Kara', 'Bassar', 'Niamtougou', 'Kétao', 'Pagouda'],
  '801137e4-e990-4dc7-83f6-db4d9b41c42d': ['Dapaong', 'Mango', 'Cinkassé', 'Tandjouaré'],
}
...
export function MarketPricesDashboard({ onBack, cooperativeName, cardNumber }: Props) {
  const [step, setStep] = useState<'regions' | 'markets' | 'prices' | 'submit'>('regions')
  const [selectedRegion, setSelectedRegion] = useState<typeof REGIONS[0] | null>(null)
  const [selectedMarket, setSelectedMarket] = useState<string | null>(null)
  const [prices, setPrices] = useState<MarketPrice[]>([])
  const [loadingPrices, setLoadingPrices] = useState(false)
  const [submitForm, setSubmitForm] = useState({ culture_id: '', price: '', market_name: '' })
  const [submitting, setSubmitting] = useState(false)
  const [submitResult, setSubmitResult] = useState<{ ok: boolean; msg: string } | null>(null)

  const fetchPrices = useCallback(async (regionId: string, market?: string) => {
    setLoadingPrices(true)
    try {
      const res = await fetch(`/api/market-prices?region_id=${regionId}`)
      const data = await res.json()
      let filtered = data.prices ?? []
      if (market) filtered = filtered.filter((p: MarketPrice) => p.market_name === market)
      setPrices(filtered)
    } catch { setPrices([]) }
  ...
  const handleSelectMarket = (market: string) => {
    ...
    setStep('prices')
    if (selectedRegion) fetchPrices(selectedRegion.id, market)
  }
    ...
    try {
      const res = await fetch('/api/market-prices', {
        ...
        body: JSON.stringify({
          ...
          market_name: submitForm.market_name || selectedMarket || 'Non précisé',
      ...
      if (res.ok) {
        ...
        setSubmitForm({ culture_id: '', price: '', market_name: '' })
        if (selectedRegion && selectedMarket) fetchPrices(selectedRegion.id, selectedMarket)
      } else {
  ...
  const goBack = () => {
    ...
    if (step === 'prices') { setStep('markets'); return }
    if (step === 'markets') { setStep('regions'); setSelectedRegion(null); return }
    onBack()
  }
  ...
  return (
    <div className="space-y-4">
      ...
      <button onClick={goBack} className="flex items-center gap-2 text-[#4ADE80] text-sm font-medium active:opacity-70">
        ...
        {step === 'regions' ? 'Retour au menu' : step === 'markets' ? 'Régions' : step === 'prices' ? 'Marchés' : 'Prix'}
      </button>
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-white/40 text-[11px] mt-0.5">
            {step === 'regions' && 'Sélectionnez une région'}
            {step === 'markets' && `${selectedRegion?.emoji} ${selectedRegion?.name} — Choisir un marché`}
            {step === 'prices' && `📍 ${selectedMarket}`}
            {step === 'submit' && '✏️ Renseigner un prix'}
          </p>
        </div>
          ...
          {REGIONS.map((r) => (
            <button key={r.id} onClick={() => handleSelectRegion(r)} className="w-full rounded-2xl bg-white/[0.04] border border-white/[0.08] p-4 flex items-center gap-4 active:scale-[0.98] transition-transform hover:bg-white/[0.06]">
              ...
              <div className="flex-1 text-left">
                ...
                <p className="text-[11px] text-white/40">{MARKETS_BY_REGION[r.id]?.length ?? 0} marchés</p>
              </div>
      ...
      {/* Step 2: Markets */}
      {step === 'markets' && selectedRegion && (
        <div className="space-y-2">
          {(MARKETS_BY_REGION[selectedRegion.id] ?? []).map((m) => (
            <button key={m} onClick={() => handleSelectMarket(m)} className="w-full rounded-xl bg-white/[0.04] border border-white/[0.08] p-3.5 flex items-center gap-3 active:scale-[0.98] transition-transform hover:bg-white/[0.06]">
              <MapPin className="h-4 w-4 text-[#4ADE80]/60 shrink-0" />
              <span className="text-sm text-white font-medium flex-1 text-left">{m}</span>
      ...
      {step === 'prices' && (
        <div className="space-y-3">
          {loadingPrices ? (
            <div className="flex justify-center py-8"><div className="w-6 h-6 border-2 border-[#4ADE80]/30 border-t-[#4ADE80] rounded-full animate-spin" /></div>
          ) : prices.length === 0 ? (
            <div className="rounded-2xl bg-white/[0.03] border border-white/[0.06] p-6 text-center">
              <p className="text-white/50 text-sm">Aucun prix disponible pour ce marché</p>
              <p className="text-white/30 text-xs mt-1">Soyez le premier à renseigner les prix !</p>
            </div>
          ) : (
            <div className="space-y-2">
              {prices.map((p) => (
                <div key={p.id} className="rounded-xl bg-white/[0.04] border border-white/[0.08] p-3 flex items-center gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-2 mt-0.5">
                      ...
                      {p.verified && <span className="text-[8px] text-[#4ADE80]/60">✓ vérifié</span>}
                    </div>
                  </div>
          ...
          {/* Submit button */}
          <button onClick={() => { setStep('submit'); setSubmitForm(f => ({ ...f, market_name: selectedMarket ?? '' })) }} className="w-full rounded-2xl p-4 bg-gradient-to-r from-[#0A5C36] to-[#0d7a4a] border border-[#4ADE80]/20 flex items-center gap-3 active:scale-[0.98] transition-transform">
      ...
      {step === 'submit' && (
        <div className="rounded-2xl bg-white/[0.04] border border-white/[0.08] p-5 space-y-4">
          <div className="flex items-center gap-2 mb-2">
            ...
            <div>
              <p className="text-sm font-bold text-white">Nouveau prix</p>
              <p className="text-[10px] text-white/40">📍 {submitForm.market_name || selectedMarket}</p>
            </div>
          </div>
          {/* Culture grid */}
          <div>
            ...
            <div className="grid grid-cols-5 gap-1.5">
              {CULTURE_LIST.map((c) => (
                <button key={c.id} onClick={() => setSubmitForm(f => ({ ...f, culture_id: c.id }))} className={`flex flex-col items-center gap-0.5 p-2 rounded-xl border transition-all ${submitForm.culture_id === c.id ? 'bg-[#4ADE80]/10 border-[#4ADE80]/40' : 'bg-white/[0.02] border-white/[0.06] active:bg-white/[0.05]'}`}>
                  <span className="text-lg">{c.emoji}</span>
                  <span className="text-[8px] text-white/60 leading-tight text-center">{c.name}</span>
                </button>
              ))}
            </div>
          </div>
          ...
          <div>
            ...
            <input type="number" inputMode="numeric" placeholder="Ex: 450" value={submitForm.price} onChange={(e) => setSubmitForm(f => ({ ...f, price: e.target.value }))} className="w-full rounded-xl bg-white/[0.06] border border-white/[0.1] px-4 py-3 text-white text-lg font-bold placeholder:text-white/20 focus:outline-none focus:border-[#4ADE80]/40 transition-colors" />
          </div>
          {/* Submit */}
          <button onClick={handleSubmitPrice} disabled={!submitForm.culture_id || !submitForm.price || submitting} className="w-full py-3.5 rounded-xl bg-[#4ADE80] text-[#0A2E1A] font-bold text-sm flex items-center justify-center gap-2 disabled:opacity-40 active:scale-[0.97] transition-transform">
            {submitting ? <div className="w-4 h-4 border-2 border-[#0A2E1A]/30 border-t-[#0A2E1A] rounded-full animate-spin" /> : <>✓ Envoyer le prix</>}
          </button>
          {submitResult && (
            <div className={`rounded-xl p-3 text-center text-xs font-medium ${submitResult.ok ? 'bg-[#4ADE80]/10 text-[#4ADE80]' : 'bg-red-500/10 text-red-400'}`}>{submitResult.msg}</div>
          )}
          <p className="text-[9px] text-white/25 text-center">Votre prix sera vérifié par la coopérative avant publication</p>
        </div>
      )}
    </div>
  )
