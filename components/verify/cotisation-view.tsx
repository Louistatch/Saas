...
export function CotisationView({
  cardNumber,
  onBack,
  coordoPhone,
  coordoName,
  ...
  const payeeCount = allCotisations.filter(
        ...
        <>
          ...
          <div className="vfp-card rounded-2xl p-5 space-y-3">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className={`text-sm font-medium ${statusInfo?.color ?? 'text-white/50'}`}>
                  ...
                  {summary.is_overdue &&
                    summary.due_date &&
                    ` — dûe le ${new Date(summary.due_date).toLocaleDateString('fr-FR')}`}
                  {summary.is_paid &&
                    summary.paid_date &&
                    ` le ${new Date(summary.paid_date).toLocaleDateString('fr-FR')}`}
                </p>
              </div>
            </div>
            ...
            {(summary.last_status === 'pending' || summary.is_overdue) && (
              <div
                className={`rounded-xl border p-3 ${summary.is_overdue ? 'border-red-500/30 bg-red-500/8' : 'border-amber-500/30 bg-amber-500/8'}`}
              >
                <p
                  className={`text-xs font-semibold mb-1 ${summary.is_overdue ? 'text-red-300' : 'text-amber-300'}`}
                >
                    ...
                    return (
                      ...
                      >
                        ...
                        <p className="text-[10px] text-white/50 text-center leading-tight">
                          {c.campaign?.replace(/^(Campagne\s+)?/i, '') ?? '—'}
                        </p>
                        {c.amount != null && (
          ...
          {tauxRegularite !== null && totalCampagnes >= 2 && (
            <div className="rounded-xl bg-white/[0.03] border border-white/[0.06] p-3">
              <div className="flex items-center justify-between mb-2">
                ...
                <span
                  className={`text-sm font-bold ${tauxRegularite >= 80 ? 'text-emerald-400' : tauxRegularite >= 50 ? 'text-amber-400' : 'text-red-400'}`}
                >
                  {tauxRegularite}%
                </span>
              </div>
              ...
              <p className="text-white/30 text-[11px] mt-1">
                {payeeCount} payée{payeeCount > 1 ? 's' : ''} sur {totalCampagnes} campagne
                {totalCampagnes > 1 ? 's' : ''}
              </p>
            </div>
          )}
            ...
            <div className="space-y-2">
              ...
              {(cotisations as Cotisation[]).slice(1).map((c, i) => {
                ...
                return (
                  <div
                    key={c.id ?? i}
                    className="vfp-card rounded-xl p-3 flex items-center justify-between"
                  >
                    <div>
