...
const REGIONS: RegionDef[] = [
  {
    name: 'Savanes',
    prefectures: ['Dapaong', 'Tone', 'Kpendjal', 'Oti'],
    x: 50,
    y: 10,
    w: 200,
    h: 80,
  ...
  {
    name: 'Kara',
    prefectures: ['Kara', 'Kozah', 'Binah', 'Dankpen', 'Doufelgou', 'Keran'],
    x: 50,
    y: 95,
    w: 200,
    h: 80,
  ...
  {
    name: 'Centrale',
    prefectures: ['Sokodé', 'Tchaoudjo', 'Tchamba', 'Sotouboua', 'Blitta'],
    x: 50,
    y: 180,
    w: 200,
    h: 80,
  ...
  {
    name: 'Plateaux',
    prefectures: ['Atakpamé', 'Ogou', 'Haho', 'Kloto', 'Wawa', 'Amou', 'Agou'],
    x: 50,
    y: 265,
    w: 200,
    h: 100,
  ...
  {
    name: 'Maritime',
    prefectures: ['Lomé', 'Golfe', 'Lacs', 'Vo', 'Yoto', 'Zio', 'Ave'],
    x: 50,
    y: 370,
    w: 200,
    h: 80,
...
function prefectureToRegion(prefecture: string): string {
  for (const r of REGIONS) {
    if (
      r.prefectures.some(
        (p) =>
          prefecture.toLowerCase().includes(p.toLowerCase()) ||
          p.toLowerCase().includes(prefecture.toLowerCase()),
      )
    ) {
      return r.name
    }
  ...
  return (
    <div className="relative">
      <svg
        ...
        role="img"
        aria-labelledby="togo-map-title"
      >
        <title id="togo-map-title">Répartition par région — Togo</title>
        {REGIONS.map((r) => {
        ...
        <text x="150" y="450" textAnchor="middle" fontSize={9} fill="#6b7280">
          Golfe du Bénin
        </text>
      </svg>
      {/* Legend */}
        ...
        <div className="flex h-3">
          {[0.1, 0.3, 0.5, 0.7, 0.9].map((r) => (
            <div
              key={r}
              className="w-6 h-3"
              style={{ background: intensityToColor(r * max, max) }}
            />
          ))}
        </div>
        <span>{max}</span>
