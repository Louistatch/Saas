...
import { generatePaymentReference, initiateOrangeMoneyPayment } from '@/lib/payments/orange-money'
...
export async function POST(req: NextRequest): Promise<NextResponse> {
  ...
  const { data: memberRow } = await session
    .from('members')
    .select('id')
    .eq('id', member_id)
  ...
  if (cotisation_id) {
    const { data: cotisation } = await session
      .from('cotisations')
    ...
    if (
      ...
      cotisation.status === 'waived' ||
      Number(cotisation.amount) !== amount_fcfa ||
      cotisation.currency !== 'XOF'
    ) {
  ...
  if (['moov', 'tmoney'].includes(provider) && amount_fcfa % 5 !== 0) {
    return NextResponse.json(
      { error: 'Le montant Mobile Money doit être un multiple de 5 FCFA' },
      { status: 400 },
    )
  }
