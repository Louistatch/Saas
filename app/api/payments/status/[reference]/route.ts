...
export async function GET(
  ...
  if (!tenant.ok) {
    const { data: member } = await supabase.from('members').select('email, cooperative_id')
      .eq('id', payment.member_id ?? '').maybeSingle()
    const { data: profile } = await supabase.from('profiles').select('role, cooperative_id').eq('id', user.id).maybeSingle()
    const ownMember = profile?.role === 'member' && !!user.email_confirmed_at && !!user.email && !!member?.email
