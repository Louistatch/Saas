    ...
    <div className="min-h-screen bg-gradient-to-b from-[#0A2E1A] via-[#0A3D22] to-[#061a0f] relative overflow-hidden">
      ...
      <div className="relative z-10 max-w-md mx-auto px-4 py-6 space-y-5">
        ...
        {isValid && activeView === 'prices' && (
          <MarketPricesDashboard onBack={() => setActiveView('menu')} cooperativeName={result.cooperative?.name} />
        )}
...
const CULTURES: Culture[] = [
  { id: 'tomate', name: 'Tomate', emoji: '🍅', season: 'oct-mars', prices: [{ market: 'Lomé', price: 450, trend: 'up' }, { market: 'Kara', price: 380, trend: 'stable' }, { market: 'Sokodé', price: 350, trend: 'down' }] },
  { id: 'oignon', name: 'Oignon', emoji: '🧅', season: 'nov-avr', prices: [{ market: 'Lomé', price: 600, trend: 'up' }, { market: 'Dapaong', price: 420, trend: 'stable' }, { market: 'Kara', price: 500, trend: 'up' }] },
  { id: 'piment', name: 'Piment', emoji: '🌶️', season: 'toute année', prices: [{ market: 'Lomé', price: 1200, trend: 'stable' }, { market: 'Kpalimé', price: 900, trend: 'down' }, { market: 'Atakpamé', price: 950, trend: 'stable' }] },
  { id: 'gombo', name: 'Gombo', emoji: '🥒', season: 'mai-oct', prices: [{ market: 'Lomé', price: 550, trend: 'up' }, { market: 'Sokodé', price: 400, trend: 'stable' }, { market: 'Kara', price: 420, trend: 'up' }] },
  { id: 'laitue', name: 'Laitue', emoji: '🥬', season: 'nov-mars', prices: [{ market: 'Lomé', price: 300, trend: 'stable' }, { market: 'Kpalimé', price: 250, trend: 'down' }, { market: 'Tsévié', price: 280, trend: 'stable' }] },
  { id: 'carotte', name: 'Carotte', emoji: '🥕', season: 'nov-mars', prices: [{ market: 'Lomé', price: 500, trend: 'up' }, { market: 'Kara', price: 450, trend: 'stable' }, { market: 'Dapaong', price: 480, trend: 'up' }] },
  { id: 'chou', name: 'Chou', emoji: '🥦', season: 'oct-fév', prices: [{ market: 'Lomé', price: 350, trend: 'stable' }, { market: 'Kpalimé', price: 280, trend: 'down' }, { market: 'Sokodé', price: 320, trend: 'stable' }] },
  { id: 'concombre', name: 'Concombre', emoji: '🥒', season: 'toute année', prices: [{ market: 'Lomé', price: 250, trend: 'down' }, { market: 'Tsévié', price: 200, trend: 'stable' }, { market: 'Atakpamé', price: 220, trend: 'stable' }] },
  { id: 'aubergine', name: 'Aubergine', emoji: '🍆', season: 'toute année', prices: [{ market: 'Lomé', price: 400, trend: 'stable' }, { market: 'Kara', price: 320, trend: 'up' }, { market: 'Sokodé', price: 350, trend: 'stable' }] },
  { id: 'poivron', name: 'Poivron', emoji: '🫑', season: 'oct-mars', prices: [{ market: 'Lomé', price: 800, trend: 'up' }, { market: 'Kpalimé', price: 650, trend: 'stable' }, { market: 'Kara', price: 700, trend: 'up' }] },
]
...
function MarketPricesDashboard({ onBack, cooperativeName }: { onBack: () => void; cooperativeName?: string }) {
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-white/40 text-[11px] mt-0.5">
            10 cultures maraîchères • Mis à jour par les coopératives
          </p>
        </div>
      ...
      {!selectedCulture && (
        <div className="grid grid-cols-2 gap-2">
          {CULTURES.map((culture) => {
            const bestPrice = Math.max(...culture.prices.map(p => p.price))
            const lowestPrice = Math.min(...culture.prices.map(p => p.price))
            return (
              <button
                key={culture.id}
                onClick={() => setSelectedCulture(culture)}
                className="rounded-2xl bg-white/[0.04] border border-white/[0.08] p-3 text-left active:scale-[0.97] transition-transform hover:bg-white/[0.06]"
              >
                <div className="flex items-center gap-2 mb-2">
                  <span className="text-xl">{culture.emoji}</span>
                  <span className="text-sm font-semibold text-white">{culture.name}</span>
                </div>
                <div className="flex items-center justify-between">
                  <div>
                    <p className="text-[10px] text-white/40">Min</p>
                    <p className="text-xs font-bold text-white">{lowestPrice} F</p>
                  </div>
                  <div className="text-right">
                    <p className="text-[10px] text-white/40">Max</p>
                    <p className="text-xs font-bold text-[#4ADE80]">{bestPrice} F</p>
                  </div>
                </div>
                <p className="text-[9px] text-white/30 mt-1.5">🗓 {culture.season}</p>
              </button>
            )
          })}
        </div>
      ...
      {selectedCulture && (
        <div className="space-y-3">
          <button
            ...
            className="flex items-center gap-2 text-white/60 text-xs active:opacity-70"
          >
            ← Toutes les cultures
          </button>
          ...
          <div className="rounded-2xl bg-white/[0.04] border border-white/[0.08] p-4">
            <div className="flex items-center gap-3 mb-4">
              <span className="text-3xl">{selectedCulture.emoji}</span>
              <div>
                <h4 className="text-white font-bold text-lg">{selectedCulture.name}</h4>
                <p className="text-white/40 text-xs">Saison : {selectedCulture.season}</p>
              </div>
            </div>

            {/* Price table */}
            <div className="space-y-2">
              <div className="grid grid-cols-3 gap-2 px-2 pb-1 border-b border-white/[0.06]">
                <span className="text-[9px] text-white/40 uppercase font-semibold">Marché</span>
                <span className="text-[9px] text-white/40 uppercase font-semibold text-right">Prix/kg</span>
                <span className="text-[9px] text-white/40 uppercase font-semibold text-right">Tendance</span>
              </div>
              {selectedCulture.prices.map((p, i) => (
                <div key={i} className="grid grid-cols-3 gap-2 px-2 py-2 rounded-lg bg-white/[0.02]">
                  <span className="text-xs text-white font-medium">{p.market}</span>
                  <span className="text-xs text-white font-bold text-right">{p.price} F</span>
            ...
            <p className="text-[9px] text-white/25 mt-3 text-center">
              Dernière mise à jour : mai 2026 • Source : coopératives FENOMAT
            </p>
          </div>
        </div>
      )}
      ...
      >
        <div className="w-10 h-10 rounded-xl bg-[#4ADE80]/15 flex items-center justify-center shrink-0">
          <PhoneCall className="h-5 w-5 text-[#4ADE80]" />
        </div>
        <div className="flex-1 text-left">
          <p className="text-sm font-semibold text-white">Renseigner les prix</p>
          <p className="text-[11px] text-white/50">Contacter {cooperativeName ?? 'ma coopérative'} pour mettre à jour</p>
        </div>
        <svg className="h-4 w-4 text-white/40 shrink-0" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
      ...
      {showContact && (
        <div className="rounded-2xl bg-white/[0.04] border border-white/[0.08] p-4 space-y-3">
          <p className="text-white text-sm font-semibold">📞 Contacter votre coopérative</p>
          <p className="text-white/50 text-xs leading-relaxed">
            Pour mettre à jour les prix du marché, contactez votre responsable de coopérative.
            Les prix sont collectés chaque semaine sur les marchés locaux et mis à jour dans le système.
          </p>
          <div className="grid grid-cols-2 gap-2">
            <a
              href="tel:+22890000000"
              className="flex items-center justify-center gap-2 py-3 rounded-xl bg-[#4ADE80]/10 border border-[#4ADE80]/20 text-[#4ADE80] text-xs font-semibold active:scale-[0.97] transition-transform"
            >
              <PhoneCall className="h-3.5 w-3.5" />
              Appeler
            </a>
            <a
              href="https://wa.me/22890000000?text=Bonjour%2C%20je%20souhaite%20renseigner%20les%20prix%20du%20march%C3%A9"
              target="_blank"
              rel="noopener noreferrer"
              className="flex items-center justify-center gap-2 py-3 rounded-xl bg-green-600/10 border border-green-500/20 text-green-400 text-xs font-semibold active:scale-[0.97] transition-transform"
            >
              💬 WhatsApp
            </a>
          </div>
          <p className="text-[9px] text-white/25 text-center">
            Les prix seront vérifiés et publiés sous 24h
          </p>
        </div>
      )}
    </div>
