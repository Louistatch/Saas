...
export default function AgriCreditPage() {
  ...
  const handleSubmit = async () => {
    ...
    const res = await fetch('/api/credit/applications', {
      ...
      body: JSON.stringify({ ...form, cooperative_id: currentCooperative.id }),
    })
    const d = await res.json()
    setSubmitting(false)
    if (res.ok) {
      ...
      toast({
        title: 'Erreur',
        description: d.error ?? 'Impossible de soumettre la demande',
        variant: 'destructive',
      })
    }
  }
  ...
  const handlePay = async (repaymentId: string) => {
    ...
    const res = await fetch(`/api/credit/applications/${selected.id}/repayments`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ repayment_id: repaymentId, amount_paid: Number(payAmount) }),
    })
    if (!res.ok) {
      toast({
        title: 'Erreur',
  ...
  const active = applications.filter((a) =>
    ['pending', 'scoring', 'approved', 'disbursed', 'repaying'].includes(a.status),
  )
  const totalEncours = applications
    .filter((a) => a.status === 'disbursed' || a.status === 'repaying')
    .reduce((s, a) => s + (a.amount_approved_fcfa ?? 0), 0)
            ...
            <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
              <DialogTrigger asChild>
                <Button className="gap-2">
                  <CreditCard className="h-4 w-4" /> Nouvelle demande
                </Button>
              </DialogTrigger>
              <DialogContent className="max-w-lg">
                <DialogHeader>
                  <DialogTitle>Demande de crédit</DialogTitle>
                </DialogHeader>
                {!scoring ? (
                  <div className="space-y-4">
                    <div>
                      <label htmlFor="credit-member" className="text-sm font-medium">
                        Membre
                      </label>
                      <Select
                      ...
                      >
                        <SelectTrigger id="credit-member">
                    ...
                    <div>
                      ...
                      <Input
                        id="credit-amount"
                        type="number"
                        value={form.amount_requested_fcfa}
                        onChange={(e) =>
                          setForm((f) => ({ ...f, amount_requested_fcfa: e.target.value }))
                        }
                      />
                    </div>
                    <div>
                      <label htmlFor="credit-purpose" className="text-sm font-medium">
                        Objet
                      </label>
                      <Select
                        value={form.purpose}
                      ...
                      >
                        <SelectTrigger id="credit-purpose">
                          <SelectValue />
                        </SelectTrigger>
                        <SelectContent>
                          {[
                    ...
                    <div>
                      <label htmlFor="credit-duration" className="text-sm font-medium">
                        Durée (mois)
                      </label>
                      <Select
                        value={form.duration_months}
                        onValueChange={(v) => setForm((f) => ({ ...f, duration_months: v }))}
                      >
                        <SelectTrigger id="credit-duration">
                          <SelectValue />
                        </SelectTrigger>
                        <SelectContent>
                          {[3, 6, 12, 18, 24].map((d) => (
                    ...
                    <Button onClick={handleSubmit} disabled={submitting} className="w-full">
                      {submitting ? 'Analyse en cours...' : 'Analyser le score'}
                    </Button>
                  </div>
                ) : (
                  <div className="space-y-4">
                    <div className="text-center p-4 rounded-lg bg-green-50 border border-green-200">
                      ...
                      <Badge className={GRADE_COLOR[scoring.grade] ?? ''}>
                        {scoring.grade} — {scoring.grade === 'F' ? 'Refusé' : 'Approuvable'}
                      </Badge>
                      <div className="text-sm text-green-700 mt-1">
                        Max: {scoring.maxLoanFcfa.toLocaleString('fr-FR')} FCFA · Taux:{' '}
                        {scoring.interestRatePct}%
                      </div>
                    </div>
                    ...
                    <div className="flex gap-2">
                      <Button variant="outline" className="flex-1" onClick={() => setScoring(null)}>
                        Modifier
                      </Button>
                      <Button
                        className="flex-1"
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {isLoading
          ...
          : [
              ...
              {
                label: 'Encours FCFA',
                value: totalEncours.toLocaleString('fr-FR'),
                icon: TrendingUp,
      ...
      <Tabs defaultValue="demandes">
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b">
                      {['Membre', 'Montant', 'Objet', 'Durée', 'Score', 'Grade', 'Statut', ''].map(
                        (h) => (
                          <th
                            key={h}
                    ...
                    {applications.map((a) => (
                      <tr key={a.id} className="border-b hover:bg-muted/50">
                        ...
                        <td className="px-4 py-3">
                          {(a.amount_requested_fcfa ?? 0).toLocaleString('fr-FR')} F
                        </td>
                        ...
                        <td className="px-4 py-3">
                          {a.credit_grade && (
                            <Badge className={GRADE_COLOR[a.credit_grade] ?? ''}>
                              {a.credit_grade}
                            </Badge>
                          )}
                        </td>
                                ...
                                <Button
                                  ...
                                  onClick={() =>
                                    handleStatusChange(
                                      a.id,
                                      'approved',
                                      a.credit_score && a.credit_score >= 50
                                        ? a.amount_requested_fcfa
                                        : undefined,
                                    )
                                  }
                                >
          ...
          {!selected ? (
            <Card>
              <CardContent className="py-12 text-center text-muted-foreground">
                Sélectionnez une demande dans l'onglet Demandes
              </CardContent>
            </Card>
          ) : (
            <Card>
              ...
              <CardContent className="p-0">
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b">
                      {['Date', 'Dû', 'Payé', 'Statut', 'Action'].map((h) => (
                        <th
                          key={h}
                          className="text-left px-4 py-3 font-medium text-muted-foreground"
                        >
                  ...
                  <tbody>
                    {(selected.credit_repayments ?? []).map((r) => (
                      <tr key={r.id} className="border-b">
                        ...
                        <td className="px-4 py-3">
                          {(r.amount_paid_fcfa ?? 0).toLocaleString('fr-FR')} F
                        </td>
                        <td className="px-4 py-3">
                          <Badge
                            className={
                        ...
                        <td className="px-4 py-3">
                          {r.status !== 'paid' && isAdmin && (
                            <div className="flex gap-2 items-center">
                              ...
                              />
                              <Button size="sm" variant="outline" onClick={() => handlePay(r.id)}>
                                Enregistrer
                              </Button>
                            </div>
                          )}
                    ...
                    {(selected.credit_repayments?.length ?? 0) === 0 && (
                      <tr>
                        <td colSpan={5} className="px-4 py-6 text-center text-muted-foreground">
                          Pas d'échéancier — approuver et débourser d'abord
                        </td>
                      </tr>
                    )}
                  </tbody>
        ...
        <TabsContent value="analytique">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <Card>
              <CardHeader>
                <CardTitle className="text-sm">Encours total</CardTitle>
              </CardHeader>
              <CardContent>
                <div className="text-2xl font-bold text-green-700">
                  {totalEncours.toLocaleString('fr-FR')} F
                </div>
              </CardContent>
            </Card>
            <Card>
              <CardHeader>
                <CardTitle className="text-sm">Taux de recouvrement</CardTitle>
              </CardHeader>
              <CardContent>
                <Progress value={repayRate} className="h-3 mt-2" />
                <div className="text-sm mt-1 text-muted-foreground">{repayRate}%</div>
              </CardContent>
            </Card>
            <Card>
              <CardHeader>
                <CardTitle className="text-sm">Distribution grades</CardTitle>
              </CardHeader>
              <CardContent>
                {(['A', 'B', 'C', 'D', 'F'] as const).map((g) => {
                  ...
                  return (
                    <div key={g} className="flex items-center gap-2 text-sm">
                      <Badge className={GRADE_COLOR[g]}>{g}</Badge>
                      <span>
                        {cnt} demande{cnt !== 1 ? 's' : ''}
                      </span>
                    </div>
