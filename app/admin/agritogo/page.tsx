...
export default function AgriTogoAdminPage() {
  ...
  const fetchHealth = useCallback(async () => {
    ...
    try {
      setHealth(await agri<HealthData>('health'))
    } catch {
      setHealth(null)
    }
  ...
  const fetchProduits = useCallback(async () => {
    ...
    try {
      setProduits(await agri<Produit[]>('produits'))
    } catch {
      setProduits([])
    }
  ...
  const addProduit = useCallback(async () => {
    ...
    try {
      await agri('cultures', {
        method: 'POST',
        body: JSON.stringify({ nom: newNom.trim(), unite: newUnite, categorie: newCat }),
      })
  ...
  const fetchPrix = useCallback(async (page = 1) => {
    setPrixLoading(true)
    try {
      setPrixData(await agri<PrixList>(`prix-list?page=${page}`))
    } catch {
      setPrixData({})
    }
  ...
  const uploadCsv = useCallback(async () => {
    ...
    try {
      ...
      const result = await agri<{ inserted?: number; errors?: number; error?: string }>(
        'upload/prix',
        { method: 'POST', body: text, headers: { 'Content-Type': 'text/plain' } },
      )
      toast({
        title: 'Import CSV',
        description: `${result.inserted ?? 0} prix importés${result.errors ? ` (${result.errors} erreurs)` : ''}`,
      })
      setCsvFile(null)
      await fetchPrix(1)
    } catch (e) {
      toast({ title: 'Erreur CSV', description: errorMessage(e), variant: 'destructive' })
    }
    setUploading(false)
  }, [csvFile, fetchPrix, toast])

  // ── KoboCollect ─────────────────────────────────────────────────────────
  const [koboCfg, setKoboCfg] = useState<{
    configured: boolean
    base_url?: string
    token_hint?: string
  } | null>(null)
  ...
  const saveKobo = useCallback(async () => {
    setSavingKobo(true)
    try {
      const res = await agri<{ success: boolean; forms_found?: number; warning?: string }>(
        'kobo/config',
        {
          method: 'POST',
      ...
      toast({
        title: 'KoboCollect configuré',
        description:
          res.forms_found !== undefined
            ? `${res.forms_found} formulaire(s) trouvé(s)`
            : res.warning,
      })
      await fetchKobo()
    } catch (e) {
  ...
  const fetchPipeline = useCallback(async () => {
    ...
    try {
      setPipeline(await agri<PipelineStatus>('pipeline/status'))
    } catch {
      setPipeline(null)
    }
  ...
  const fetchStats = useCallback(async () => {
    try {
      setDbStats(await agri<Record<string, unknown>>('stats'))
    } catch {
      /* silent */
    }
  }, [])
    ...
    <div className="space-y-8">
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          {health ? (
            <span
              className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-semibold ${health.status === 'ok' ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}`}
            >
      ...
      <Tabs defaultValue="produits">
        <TabsList>
          <TabsTrigger value="produits" className="gap-2" onClick={fetchProduits}>
            <ShoppingCart className="h-4 w-4" />
            Produits
          </TabsTrigger>
          <TabsTrigger value="prix" className="gap-2" onClick={() => fetchPrix(1)}>
            <Activity className="h-4 w-4" />
            Prix marché
          </TabsTrigger>
          <TabsTrigger value="kobo" className="gap-2" onClick={fetchKobo}>
            <Database className="h-4 w-4" />
            KoboCollect
          </TabsTrigger>
          <TabsTrigger value="pipeline" className="gap-2" onClick={fetchPipeline}>
          ...
          <Card className="border-border">
            ...
            <CardContent>
              <div className="flex flex-col sm:flex-row gap-3">
                <Input
                  placeholder="Nom du produit (ex: Maïs)"
                  value={newNom}
                ...
                <Button
                  onClick={addProduit}
                  disabled={!newNom.trim() || busyProd === 'new'}
            ...
            <CardContent>
              {produitsLoading ? (
                ...
                <EmptyState
                  icon={ShoppingCart}
                  title="Aucun produit"
                  description="Ajoutez le premier produit ci-dessus"
                />
              ) : (
                        ...
                        <tr key={p.id} className="border-b border-border hover:bg-accent/5">
                          ...
                          <td className="py-2 px-3 text-right">
                            <Button
                              ...
                              disabled={busyProd === p.id}
                              onClick={() => deleteProduit(p.id, p.nom)}
                            >
                              {busyProd === p.id ? (
        ...
        <TabsContent value="prix" className="mt-6 space-y-6">
          <Card className="border-border">
            <CardHeader>
              ...
              <CardDescription>
                Colonnes attendues : produit, marche, prix, date (YYYY-MM-DD)
              </CardDescription>
            </CardHeader>
            <CardContent>
          ...
          <Card className="border-border">
            <CardHeader className="flex flex-row items-center justify-between">
              ...
              <Button
                size="sm"
                variant="outline"
                onClick={() => fetchPrix(prixPage)}
                disabled={prixLoading}
                className="gap-2"
              >
            ...
            <CardContent>
              {prixLoading ? (
                ...
                <EmptyState
                  icon={Activity}
                  title="Aucun prix"
                  description="Importez un CSV pour commencer"
                />
              ) : (
                <>
                  <div className="overflow-x-auto">
            ...
            <CardContent className="space-y-4">
              ...
              <div className="grid gap-3 sm:grid-cols-2">
                <div className="space-y-1">
                  <label htmlFor="kobo-url" className="text-xs font-medium text-muted-foreground">
                    URL du serveur
                  </label>
                  <Input
                    id="kobo-url"
                ...
                <div className="space-y-1">
                  <label htmlFor="kobo-token" className="text-xs font-medium text-muted-foreground">
                    Token API
                  </label>
                  <Input
                    id="kobo-token"
                    type="password"
                    placeholder="Token KoboToolbox"
                    value={koboToken}
              ...
              <Button
                onClick={saveKobo}
                disabled={!koboUrl || !koboToken || savingKobo}
                className="gap-2"
              >
            ...
            <CardContent>
              {koboLoading ? (
                ...
                <EmptyState
                  ...
                  description="Configurez KoboCollect pour voir les formulaires"
                />
              ) : (
                <div className="space-y-2">
                  {koboForms.map((f) => (
                    ...
                    >
                      ...
                      <span
                        className={`px-2 py-0.5 rounded-full text-xs font-semibold ${f.deployment_active ? 'bg-green-100 text-green-700' : 'bg-muted text-muted-foreground'}`}
                      >
          ...
          <Card className="border-border">
            <CardHeader className="flex flex-row items-center justify-between">
              <div>
                ...
                <CardDescription>
                  {pipeline
                    ? `${pipeline.ready}/${pipeline.total} fichiers présents`
                    : 'Cliquez Actualiser pour vérifier'}
                </CardDescription>
              </div>
              <Button
                size="sm"
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <p className="text-xs text-muted-foreground">
                            {present ? 'Présent' : 'Manquant — modèle dégradé'}
                          </p>
                        </div>
                      </div>
                      <span
                        className={`px-2 py-0.5 rounded-full text-xs font-semibold ${present ? 'bg-green-100 text-green-700' : 'bg-destructive/10 text-destructive'}`}
                      >
                        {present ? 'OK' : 'ABSENT'}
                      </span>
                    </div>
              ...
              {[
                {
                  ...
                  desc: 'Prévision de la volatilité des prix (séries temporelles)',
                  file: 'Core_TimeSeries',
                },
                {
                  name: 'Financial Risk',
                  desc: 'Score de risque financier des exploitations',
                  file: 'AgriRiskFin',
                },
                {
                  name: 'Farmer Segmentation',
                  desc: 'Segmentation K-Means des agriculteurs',
                  file: 'yield_df',
                },
                {
                  name: 'Crop Yield',
                  desc: 'Prédiction des rendements par culture',
                  file: 'yield_df',
                },
                { name: 'KPI Dashboard', desc: 'Indicateurs clés agrégés', file: null },
                {
                  name: 'AgriSmart Irrigation',
                  desc: 'Calcul FAO-56 des besoins en eau',
                  file: null,
                },
              ].map((m) => {
                ...
                return (
                  ...
                  >
                    <div
                      className={`h-2 w-2 rounded-full shrink-0 ${fileOk ? 'bg-green-500' : 'bg-amber-500'}`}
                    />
              ...
              {health ? (
                <div className="grid gap-3 md:grid-cols-2">
                  {[
                    {
                      label: 'Statut',
                      value: health.status === 'ok' ? '🟢 En ligne' : `🔴 ${health.status}`,
                    },
                ...
                <p className="text-sm text-muted-foreground">
                  Service non accessible. Vérifiez <code>AGRITOGO_API_URL</code>.
                </p>
              )}
            </CardContent>
          </Card>
