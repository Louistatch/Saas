...
export default async function AttestationPage({
  ...
  const { data: atsData } = await supabase.rpc('calculate_member_ats', { p_member_id: memberId })
  ...
  const atsScore =
    typeof atsData === 'number' ? atsData : Array.isArray(atsData) ? atsData[0] : null
  ...
  const location =
    [member.village, member.canton, member.prefecture, member.region].filter(Boolean).join(', ') ||
    '—'
    ...
    <>
      ...
      >
          ...
          <div style={{ flex: 1 }}>
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
              <tbody>
                ...
                <InfoRow
                  label="Statut"
                  value={<span style={{ color: GREEN, fontWeight: 'bold' }}>Actif ✓</span>}
                />
                {card && (
                  <>
        ...
        >
          <StatBox
            label="Score ATS"
            value={scoreNum !== null ? `${scoreNum}/100` : '—'}
            sub={scoreNum !== null ? getAtsLevel(scoreNum) : ''}
          />
          <StatBox
            label="Parcelles"
            value={String(totalParcelles)}
            sub={`${totalSurface.toFixed(2)} ha total`}
          />
          <StatBox
            label="Localisation"
            value={member.canton ?? member.prefecture ?? '—'}
            sub={member.region ?? ''}
          />
        </div>
        ...
        {/* Location */}
        <table
          style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13, marginBottom: 32 }}
        >
          <tbody>
        ...
        >
          <div style={{ fontSize: 12, color: '#555' }}>
            <div>
              Émis le : <strong>{issueDate}</strong>
            </div>
        ...
        >
          FaîtiereHub — Plateforme de gestion des coopératives agricoles de l&apos;Afrique de
          l&apos;Ouest
        </div>
      </div>
    </>
